...
void dfs(int loc, int parent, int mi,bool check){
	if(check){
		sum[mi]+=1;
		sum[mi]+=query(MAXN-1)-query(mi-1);
	}
	else sum[mi]+=query(MAXN-1)-query(mi);
	toadd.push_back(mi);
	for(pii x:matrix[loc]){
		if(!cent[x.first]&&x.first!=parent)dfs(x.first,loc,min(x.second,mi),check);
	}
}
