...
int main(){
    ...
    //scan(n, m, p);
    for(int i = 0; i < n; i++){
        scanf("%lld %lld", &in[i].first.first, &in[i].first.second);
        assert(in[i].first.first <= 1e6);
        //scan(in[i].first, in[i].second);
    }
    sort(in, in+n);
    for(int i = n-2; i >= 0; i--)
        in[i].first.second = min(in[i].first.second, in[i+1].first.second);
    for(int i = n-1; i >= 0; i--){
        stx[in[i].first.first] = min(stx[in[i].first.first], in[i].first.second);
    }
    ...
    for(int i = m-1; i > 0; i--){
        in[i].first.second -= in[i-1].first.second;
    }
    for(int i = 0; i < m; i++){
        v.push_back({0, in[i].first.first, 0, -in[i].first.second,i});
