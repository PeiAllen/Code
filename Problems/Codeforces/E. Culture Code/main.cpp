...
data merge(const data &a, const data &b){
    if(a.extra==b.extra)return data(a.extra,(a.am+b.am)%mod);
    else if(a.extra<b.extra) return a;
    return b;
}
struct seg{
    data t[2*MN];
    void update(int ind, int le, int ri, int loc, data val){
        ...
        t[ind]=merge(t[left],data((t[right].extra==INT_MAX?INT_MAX:t[right].extra+ins[mid+1]-ins[le]),t[right].am));
    }
    data query(int ind, int le, int ri, int l, int r){
        if(l>ri||r<le)return data();
        if(le>=l&&ri<=r)return t[ind];
        int mid=(le+ri)/2;
        int left=ind+1,right=ind+(mid-le+1)*2;
        auto te=query(right,mid+1,ri,l,r);
        return merge(query(left,le,mid,l,r),data(te.extra==INT_MAX?INT_MAX:te.extra+max(0,ins[mid+1]-ins[max(le,l)]),te.am));
    }
}tree;
int first[MN];
int main(){
    ...
    for(auto x:arr){
        ...
        data trans=tree.query(0,0,sz(ins)-1,ne,first[ne]-1);
        if(trans.extra==INT_MAX)trans=data(0,1);
