...
template<typename T> struct LCT{
    const static T defq=0,defu=LLONG_MIN,bad=LLONG_MIN;//change if T is not ll
    static T Fq(T a, T b){return a+b;}//sum for this one
    static T Fu(T a, T b){return b;}//sum for this one(need to change apply if funciton changed)
    struct node{
        int s;
        bool rev;
        T d,td,lazy;
...
int main(){
    ...
    string in;
    ll a,b;
    for(int i=1;i<=n;i++)cin>>a,tree.updatepath(i,i,a);
    int q;
    cin>>q;
    while(q--){
        cin>>in>>a>>b;
        if(in=="bridge"){
            if(tree.link(a,b))printf("yes\n");
            else printf("no\n");
        }
        else if(in=="penguins")tree.updatepath(a,a,b);
        else{
            ll ans=tree.querypath(a,b);
