...
bool unigroup(int a, int b, int col) {
    ...
    for (auto x : group[bp].second) {
        // printf("UNI groupa:%d groupb:%d eleingroupb:%d groupofleft:%d groupofright:%d col:%d\n", ap, bp, x, (x - 1 >= 1 ? findgroup(x - 1) : -1), (x + 1 <= n ? findgroup(x + 1) : -1), col);
...
int main() {
    ...
    for (int cs = 1; cs <= t; cs++) {
        ...
        while (1) {
            ...
            queries = vector<pair<pii, vector<int>>>();
            queries = nextqueries;
        }
        printf("Case #%d: %lli\n", cs, ans);
    }
