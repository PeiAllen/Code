...
int main(int argc, char* argv[]){
    ...
    int n=stoi(argv[1]),t=stoi(argv[2]),ft=stoi(argv[3]),cater=stoi(argv[4]);//t is the elongation https://codeforces.com/blog/entry/18291
    printf("%d\n",n);
    for(int i=1;i<n/2-1000;i++){
        par[i]=rnd.wnext(i,t);
    }
    for(int i=n/2-1000;i<n;i++){
        par[i]=i-1;
        if(cater){
            if(i+1<n)par[i+1]=i;
            if(i+2<n)par[i+2]=i;
