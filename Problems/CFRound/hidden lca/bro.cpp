...
int main() {
    ...
    vi cand;
    vpii vv;
    for (i = 0; i < n; i++) vv.pb(mp(adjList[i].size(),i));
    sort(vv.begin(),vv.end(),greater<pii>());
    for (i = 0; i < n; i++) cand.pb(vv[i].second);
    int k;
    for (k = 0; k < cand.size(); k++) {
//        if ((double) (clock()-init)/CLOCKS_PER_SEC > 10) break;
        if(cand[k]==n-1||cand[k]==n-2){
            printf("%d %d %d %d\n",cand[k],k,vv[k].first,vv[k].second);
            return 0;
        }
        i = cand[k];
        doDFS(i,-1);
