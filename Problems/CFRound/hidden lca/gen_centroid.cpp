...
int dfssz(int loc, int parent){
    sts[loc]=1;
    for(auto x:adj[loc])if(x!=parent)sts[loc]+=dfssz(x,loc);
    return sts[loc];
}
int getcentroid(int loc, int parent, int hfsz){
    for(auto x:adj[loc])if(x!=parent&&sts[x]>hfsz)return getcentroid(x,loc,hfsz);
    return loc;
}
int main(int argc, char* argv[]){
    ...
    int f=getcentroid(1,0,dfssz(1,0)/2);
